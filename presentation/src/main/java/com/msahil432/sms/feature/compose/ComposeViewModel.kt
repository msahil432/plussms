...
import com.msahil432.sms.filter.ContactFilter
import com.msahil432.sms.interactor.AddScheduledMessage
import com.msahil432.sms.interactor.CancelDelayedMessage
import com.msahil432.sms.interactor.ContactSync
import com.msahil432.sms.interactor.DeleteMessages
import com.msahil432.sms.interactor.MarkRead
import com.msahil432.sms.interactor.RetrySending
...
import com.msahil432.sms.model.Attachments
...
class ComposeViewModel @Inject constructor(
    ...
    init {
        ...
        disposables += conversation
                ...
                .observeOn(AndroidSchedulers.mainThread())
                .map { conversation ->
                    val messages = messageRepo.getMessages(conversation.id, "", conversation.category)
                    newState { copy(selectedConversation = conversation.id,
                            messages = Pair(conversation, messages), category = category) }
                    messages
                }
        ...
        disposables += conversation
                ...
                .distinctUntilChanged()
                .withLatestFrom(state) { id, state -> messageRepo.getMessages(id, state.query, category) }
                .switchMap { messages -> messages.asObservable() }
    ...
    override fun bindView(view: ComposeView) {
        ...
        Observables.combineLatest(view.queryChangedIntent, contacts, selectedContacts)
            { query, contacts, selectedContacts ->
        ...
        Observables
                .combineLatest(
                        ...
                        conversation.mapNotNull {
                            conversation -> conversation.takeIf { it.isValid }?.vid
                        }.distinctUntilChanged())
                { visible, threadId ->
                    when (visible) {
                        true -> {
